...
local function get_spawn_biomes(name)
	local biomes = asuna.features.animals[name]
...
local function generate_page(mob)
	...
	local page = {
		...
		{ -- Spawn Biome
			...
			size = {x = 1, y = 1},
			biome_iter = 1,
			text = biome_cubes[get_spawn_biomes(name)[1]]
		},
		{ -- Biome Label
			...
			size = {x = 1, y = 1},
			biome_iter = 1,
			text = correct_string(get_spawn_biomes(name)[1])
		},
	...
	pages = {
		...
		["animalia:tropical_fish"] = {
			...
			{ -- Spawn Biome
				...
				size = {x = 1, y = 1},
				biome_iter = 1,
				text = biome_cubes[get_spawn_biomes("tropical_fish")[1]]
			},
			{ -- Biome Label
				element_type = "tooltip",
				offset = {x = 0.825, y = 8.15},
				size = {x = 1, y = 1},
				biome_iter = 1,
				text = correct_string(get_spawn_biomes("tropical_fish")[1])
			},
			libri.render_element({ -- Health Icon
				element_type = "image",
				offset = {x = 2.535, y = 8.15},
		...
		if page ~= "home" then
			for _, info in ipairs(elements) do
				...
				if info.biome_iter then
					...
					if asuna.features.animals[mobname][info.biome_iter + 1] then
						info.biome_iter = info.biome_iter + 1
					else
						info.biome_iter = 1
					end
					local spawn_biome = asuna.features.animals[mobname][info.biome_iter] or "grassland"
